...
define
   ...
   NumberOfAgents=({OS.rand} mod 10)+1
   ...
   % PAS ENCORE UTILISEE --> PLUTOT DANS LA MAIN
   proc{GenerateNTrainers N} %Generate N (0<N<10) Trainers and put them in ListTrainers
      proc{GenerateNTrainers2 N I}
	 if I==N then skip
	 else
	    local Temp in
	       Temp={NewIA trainer(c:({OS.rand} mod map.nbLines)+1 %VERIF SI DEJA AVANT
					 r:({OS.rand} mod map.nbLines)+1 % IDEM
					 isDefeated:false
					 p1:nil
					 p2:nil  % LES REMPLIR ALEATOIREMENT
					 p3:nil) I}
	    end
	    
	    {GenerateNTrainers2 N I+1}
	 end
      end
   in
      {GenerateNTrainers2 N 1}
   end
